...
//	post 방식으로 요청될 때 자동으로 실행되는 메소드
	...
	protected void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// get, post 요청의 구분없이 실행할 작업이 있으면 실행한다.
...
//		System.out.println(uploadDirectory);
		...
		// MIME 타입을 얻어오지 못했다면 파일을 전송하는 것을 사용자에게 알려주기 위해서 response 객체를
		...
		// 파일에서 읽어들인 데이터를 저장한 배열에 배열의 시작 인덱스 부터 마지막 인덱스 까지 채우고 읽어들인
		// 크기를 리턴한다. 읽어들인 데이터가 없으면 -1을 리턴한다.
		while((data = fileInputStream.read(b, 0, b.length)) != -1) {
			 outputStream.write(b, 0, data);
		}

		// flush() 메소드로 ServeltOutputStream 객체의 출력 버퍼에 남아있는 모든 데이터를 전송하고
		// 전송에 사용한 모든 객체를 닫는다.
		outputStream.flush();
		outputStream.close();
